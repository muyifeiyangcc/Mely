...
struct ChadetliLsrfCbqFVharView: View {
  ...
  var body: some View {
    MelyYemianScaffold {
      GeometryReader { geo in
        VStack(spacing: 0) {
          MelyTopBarView(
            title: xbxUEw3vtJr82k,
            onBack: { dismiss() },
            onMoreTap: { ItVGIMtNyS1Ndc = true }
          )
  ...
  private func hpEgZ6jfUA7uLLXf(_ E6j33FwVwK28lH: UIImage) {
    guard let path = ImageStorageHelper.saveChatImage(E6j33FwVwK28lH) else { return }
...
private struct VbuVNgXh5SaakivIP: View {
  ...
  var body: some View {
    Button {
      let rs2IZOBLYd7pomvi = VoiceRecorderHelper.resolveAudioPath(ph9xca0pgqMHIkqc)
